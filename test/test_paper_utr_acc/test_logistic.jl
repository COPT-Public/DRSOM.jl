...
if bool_plot
    ...
    maxstep = 1200
    colors = palette(:Paired_8)[[1, 1, 2, 3, 4, 5]]
    markers = [:circle, :rect, :circle, :circle, :circle, :circle]
    for (k, (nm, rv)) in enumerate(results)
        yv = getresultfield(rv, metric)
        maxlength = min(yv |> length, maxstep)
        indices = 1:maxlength
        # yv .- f₊ .+ 1e-20,
        yv = yv[indices]
        # xv = 1:(yv|>length)
        xv = getresultfield(rv, :kH)[indices]
