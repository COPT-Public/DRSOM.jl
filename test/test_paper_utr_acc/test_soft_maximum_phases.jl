...
if bool_opt
    ...
    for pk in [0.7, 0.81, 1.1, 2.5]
        ...
        push!(results, ("ATR-$(ratio_σ)", round(pk; digits=1), rd))
    end
...
if bool_plot
    ...
    for xaxis in [:k, :t]
        ...
        for (k, (nm, ratio_σ, rv)) in enumerate(results)
            ratio = round(rv.state.acc_count[:III] / sum(values(rv.state.acc_count)); digits=3)
            yv = getresultfield(rv, metric)
            @info "plotting $metric"
            plot!(fig,
                xaxis == :t ? getresultfield(rv, :t) : (1:(yv|>length)),
                yv .- f₊ .+ 1e-20,
                label=L"$\sigma$:=%$(ratio_σ) (%$(ratio))",
                linewidth=2.5,
                # markershape=:circle,
                # markersize=2.0,
                # markercolor=:match,
