...
if bool_plot
    ...
    for xaxis in [:k]
        ...
        fig = plot(
            ...
            titlefontsize=22,
            palette=palette(:Paired_8)[[1, 3, 5, 7]] #palette(:PRGn)[[1,3,9,10,11]]
        )
        init = 5
        for (k, (nm, rv)) in enumerate(results)
            if nm == "CubicReg" || nm == "CubicReg-A"
                yv = getresultfield(rv, :θ) ./ getresultfield(rv, :Δ)
                yv = yv[init:init+150]
                yv = yv .* 150 / sum(yv)
            else
                yv = (getresultfield(rv, :θ) + getresultfield(rv, :σ) .* sqrt.(getresultfield(rv, :ϵ))) ./ getresultfield(rv, :Δₙ)
                yv = yv[init:init+150]
                yv = yv .* 150 / sum(yv)
